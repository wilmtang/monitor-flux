...
- `Support/PlaceIndex.swift`: pure offline city/ZIP search behind the Follow-sunset **Location**
  row (`Views/LocationField.swift`). Reads the committed `Resources/places.tsv` (regenerated by
  `script/make_place_index.swift` from GeoNames + the US Census ZCTA gazetteer): diacritic-folded
  prefix search ranked by population, 5-digit-ZIP and raw-`lat,lon` classification, and
  `nearest(...)` to name a Core Location fix (and borrow its IANA timezone) with no reverse
  geocode. Loaded once off-main via `AppStore.loadedPlaceIndex`. **No geocoding service** — this
  is what keeps the schedule "computed on-device"; MapKit/`CLGeocoder` were rejected (network,
  and `CLGeocoder` is deprecated below the macOS 26 `MKGeocodingRequest`). See `docs/DESIGN.md`.
- `Support/LocationStaleness.swift`: pure traveling detection — `check(placeZoneID:systemZone:
  now:)` warns only when the schedule's place and the Mac's clock differ by ≥1 h **at now**
  (offsets, not zone ids, so Madrid/Paris stay quiet and Phoenix/Denver differ only in summer).
  Drives `AppStore.locationMismatch` → a dismissible `WarningCard`; a searched location is never
  overwritten automatically (`locationFollowsDevice` gates the every-launch Core Location re-fix).
...
- **Launch hooks (env vars), set under `MONITORFLUX_SAFE_MODE=1`:**
  ...
  - `MONITORFLUX_LOCATION_DEMO=<query>` (in-memory only) puts the Schedule pane in Follow-sunset
    with that place pinned as if picked in the location search — `tokyo` trips the traveling hint
    on a Pacific-time Mac. `MONITORFLUX_LOCATION_QUERY=<prefix>` opens the Location row mid-search
    showing the suggestion list. Pair with `MONITORFLUX_SELECT=color MONITORFLUX_SCROLL_TO=schedule`
    to frame the rows for a snapshot.
